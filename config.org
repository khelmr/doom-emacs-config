...
Use visual line numbers on org and markdown modes.
#+begin_src emacs-lisp :tangle yes
(add-hook! visual-line-mode (setq display-line-numbers-type 'visual))
#+end_src

Make evil mode respect visual line movement (taken from https://github.com/librephoenix/nixos-config/blob/main/user/app/doom-emacs/config.el)
...
#+end_src
** Window
Prompt consult buffer upon splitting a new window (taken from https://tecosaur.github.io/emacs-config/config.html#windows)
#+begin_src emacs-lisp :tangle yes
...
#+end_src
*** Appearance
Add back support for latex ligatures
#+begin_src emacs-lisp :tangle yes
(after! org
...
Enable mixed-pitch mode with org
#+begin_src emacs-lisp :tangle yes
...
#+begin_src emacs-lisp :tangle no
(after! org-capture
  ;; change capture targets to a common inbox
...
**** Doct
Improve capture template creation experience with doct.
