...
(add-hook! '+doom-dashboard-mode-hook #'+doom-dashboard-randomize-splash)
(advice-add '+doom-dashboard-randomize-splash :after #'+doom-dashboard-reload)
#+end_src
...
(defun +doom-dashboard-flower ()
  ...
  (insert
   ...
   (+doom-dashboard--center
    ...
    (with-temp-buffer
      (insert-text-button (propertize "🌸" 'face '(:height 200))
                          'action (lambda (_) (+doom-dashboard-reload t)))
      (buffer-string)))))
#+end_src
...
#+RESULTS:
| doom-dashboard-widget-banner | doom-dashboard-widget-loaded | splash-phrase-dashboard-insert |
...
#+begin_src emacs-lisp :tangle no
(after! (:and org doct)
  (setq (doct '(("Inbox" :keys "i" :file (file "inbox.org"))))))
#+end_src
